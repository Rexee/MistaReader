 ...
 */
package com.mistareader.util.views.frescoZoomable;
...
/**
 * AnimatedZoomableController that adds animation capabilities to DefaultZoomableController using standard
 * Android animation classes
 */
public class ZoomableController implements TransformGestureDetector.Listener {

    private static final Class<?> TAG = ZoomableController.class;
    ...
    private float mMidScaleFactor = 3.0f;
    ...
    public static ZoomableController newInstance() {
        return new ZoomableController(TransformGestureDetector.newInstance());
    }

    @SuppressLint("NewApi")
    public ZoomableController(TransformGestureDetector transformGestureDetector) {
        mGestureDetector = transformGestureDetector;
    ...
    public boolean isIdentity() {
        return !isAnimating() && isMatrixIdentity(mActiveTransform, 1e-3f);
    }
    ...
    public void setMidScaleFactor(float midScaleFactor) {
        mMidScaleFactor = midScaleFactor;
    }
    ...
    protected boolean calculateZoomToPointTransform(
            ...
            PointF imagePoint,
            PointF viewPoint,
            @ZoomableController.LimitFlag int limitFlags) {
  ...
  /* TransformGestureDetector.Listener methods  */
    ...
    protected boolean calculateGestureTransform(
            Matrix outTransform,
            @ZoomableController.LimitFlag int limitTypes) {
     ...
     * @return whether limiting has been applied or not
     */
    private boolean limitTranslation(Matrix transform, @ZoomableController.LimitFlag int limitTypes) {
     ...
     */
    private static boolean shouldLimit(@ZoomableController.LimitFlag int limits, @ZoomableController.LimitFlag int flag) {
